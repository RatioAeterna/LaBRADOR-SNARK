...
impl Verifier {
    ...
    pub fn verify(&self, st: &State, proof : &Transcript, crs : &CRS) -> bool {
        ...
        if is_verbose() {println!("Lines 1 and 2... enumeration of the state and the proof transcript");}

        if is_verbose() {println!("starting line 3");}
        // LINE 3
        ...
        if is_verbose() {println!("starting line 4");}
        // LINE 4
        ...
        if is_verbose() {println!("starting line 5");}
        // LINE 5
        ...
        if is_verbose() {println!("starting line 6");}
        // LINE 6 
        ...
        if is_verbose() {println!("starting line 7");}
        // LINE 7
        ...
        if is_verbose() {println!("starting line 8");}
        // CHECK 8
        ...
        if is_verbose() {println!("starting line 9");}
        // CHECK 9
        ...
        if is_verbose() {println!("starting line 12");}
        // LINE 12
        // Decompose matrix elements g_ij
        let mut g_mat_decompositions = Array2::from_elem((R,R), vec![]); 
        for i in 0..R {
            for j in 0..R {
                //println!("g_mat term at [i={},j={}]... {}", i,j,&proof.g_mat[[i,j]]);
                let dec_gij: Vec<Rq> = decompose_polynomial(&proof.g_mat[[i,j]], *B_2, *T_2);
        ...
        if is_verbose() {println!("starting line 13");}
        // LINE 13
        // Decompose matrix elements h_ij
        let mut h_mat_decompositions = Array2::from_elem((R,R), vec![]); 
        for i in 0..R {
            for j in 0..R {
                let dec_hij: Vec<Rq> = decompose_polynomial(&proof.h_mat[[i,j]], *B_1, *T_1);
                h_mat_decompositions[[i,j]] = dec_hij;
            }
        }


        if is_verbose() {println!("starting line 14");}
        // LINE 14
        ...
        if is_verbose() {println!("starting line 15");}

        // CHECK 15
        let lhs = polynomial_matrix_product(&crs.a_mat, &vec_to_column_array(&proof.z)).column(0).to_vec();
        ...
        if is_verbose() {println!("starting line 16");}
        ...
        if is_verbose() {println!("starting line 17");}
        // CHECK 17
        ...
        if is_verbose() {println!("starting line 18");}
        // CHECK 18
        ...
        if is_verbose() {println!("starting line 19");}
        // CHECK 19
        ...
        if is_verbose() {println!("starting line 20");}
        // CHECK 20
    ...
    pub fn valid_projection(&mut self, projection: &Vec<i128>) -> bool {
        ...
        if is_verbose() {println!("TOTAL NORM OF JL PROJECTION: {}, {}", norm, val.sqrt()*(*BETA_BOUND as f64));}
