...
pub fn proof_gen(S: Array2<Polynomial<i64>>) {
    ...
    let mut projection = Array2::zeros((256,1));
    for i in 0..R {
        let pi_i = sample_jl_projection();
        let product : Array2<Polynomial<i64>> = pi_i * S.column(i);
        projection += product;
    }
    // now, verifier checks whether this is legit.
    //assert!(valid_projection(projection), "Verifier rejected JL projection!");
}
