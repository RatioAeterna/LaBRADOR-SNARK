...
impl<'a> Prover<'a> {
    ...
    pub fn new(S: &'a Array2<Polynomial<i128>>, verifier: &'a mut Verifier) -> Self {
        Prover {
            S,
            verifier,
        }
    ...
    pub fn proof_gen(&mut self, st : &State, crs : &CRS) -> Transcript {
        //let t = Array2::<i128>::zeros((M,R)); // Inner commitment vector 
        //let t = Array2<Polynomial<i128>>::zeros((R,N)); // Inner commitment vector ( After looking at this some more, I think this is the actual dimension. Unclear, however. )
        
        //let zero_poly = Polynomial::new(vec![0i128]);
        //let mut t = Array2::from_elem((R,N), zero_poly);
        let mut t_i_all : Vec<Vec<Polynomial<i128>>> = vec![];
        ...
        for i in 0..R {
            for j in 0..R {
                ...
                let res : Polynomial<i128> = polynomial_vec_inner_product(&col_i, &col_j);
        ...
        for i in 0..R {
            let t_i_decomposed : Vec<Vec<Polynomial<i128>>> = decompose_polynomial_vec(&t_i_all[i], *B_1, *T_1);
        ...
        for i in 0..R {
            for j in i..R {
                let g_ij : Vec<Polynomial<i128>> = decompose_polynomial(&Gij[[i,j]], *B_2, *T_2);
                for k in 0..(*T_2 as usize) {
                    let C_ijk = crs.C_mat.get(&(i,j,k)).unwrap().column(0).to_vec();
                    let g_ij_k : &Polynomial<i128> = &g_ij[k];
        ...
        // case.
        let u_1 : Vec<Polynomial<i128>> = add_poly_vec(&lhs, &rhs);
        ...
        let mut psi : Vec<Vec<i128>> = vec![];
        let mut omega : Vec<Vec<i128>> = vec![];


        // TODO these comments aren't precise enough in dimensions
        // vector containing all phi'' for k in 1..(upper_bound), i in 0..(R-1)
        // TODO also perhaps switch this to an actual Vec<Array2<Polynomial<i128>>> later.
        let mut phi_prime_prime_vec : Vec<Vec<Vec<Polynomial<i128>>>> = vec![];

        // vector containing all b'' for k in 1..(upper_bound)
        let mut b_prime_prime_vec : Vec<Polynomial<i128>> = vec![];
        ...
        for k in 1..(upper_bound+1) {
            ...
            let mut phi_prime_prime_k : Vec<Vec<Polynomial<i128>>> = vec![];
            ...
            let mut a_prime_prime : Array2<Polynomial<i128>> = Array2::zeros((R,R));

            for i in 0..R {
                let phi_i_prime_prime : Vec<Polynomial<i128>> = vec![];
                for j in 0..R {
                    let a_prime_ij : &Polynomial<i128> = &st.a_prime_k[0][[i,j]];
                    let a_prime_prime_ij : Polynomial<i128> = multiply_poly_ints(&a_prime_ij, &psi.last().unwrap());
                    ...
                    let phi_prime_i : Vec<Polynomial<i128>> = st.phi_k[0].column(i).to_vec();


                    let mut lhs : Vec<Polynomial<i128>> = gen_empty_poly_vec(N);
                    ...
                    let mut rhs : Vec<Polynomial<i128>> = vec![Polynomial::new(vec![]); N as usize];

                    for j in 0..256 {
                        let bolded_pi_poly_vec : Vec<Polynomial<i128>> = concat_coeff_reduction(&Pi_i.row(j).to_vec());
            ...
            let mut lhs = Polynomial::new(vec![0i128]);
            for i in 0..R {
                for j in 0..R {
            ...
            let mut rhs = Polynomial::new(vec![0i128]);
            for i in 0..R {
        ...
        let alpha : Vec<Polynomial<i128>> = self.verifier.fetch_alpha();
        let beta : Vec<Polynomial<i128>> = self.verifier.fetch_beta();

        let mut phi_final : Vec<Vec<Polynomial<i128>>> = vec![];
        for i in 0..R {
            let mut lhs : Vec<Polynomial<i128>> = gen_empty_poly_vec(N);
            for k in 0..K {
            ...
            let mut rhs : Vec<Polynomial<i128>> = gen_empty_poly_vec(N);
        ...
        // Compute u_2
        let mut u_2 : Vec<Polynomial<i128>> = vec![Polynomial::<i128>::new(vec![]); KAPPA_2 as usize];
        for i in 1..(R+1) {
            for j in i..(R+1) {
                for k in 0..(*T_1 as usize) {
        ...
        let mut z : Vec<Polynomial<i128>> = vec![Polynomial::new(vec![]); R as usize];
        let mut c_vec : Vec<Polynomial<i128>> = vec![];

        for i in 0..R {
            let c_i = self.verifier.fetch_challenge();
            c_vec.push(c_i);
            let prod : Vec<Polynomial<i128>> = poly_by_poly_vec(c_vec.last().as_ref().unwrap(), &self.S.column(i).to_vec());
            for j in 0..R {
                z[j] = &z[j] + &prod[j];
            }
        }
    ...
    pub fn jl_project(&mut self) -> Vec<i128> {
        ...
        for i in 0..R {
            ...
            let s_i_coeffs : Array2<i128> = vec_to_column_array(&witness_coeff_concat(&self.S.column(i).to_vec()));
...
pub fn generate_witness() -> Array2<Polynomial<i128>> {
    ...
    if norm_sum > (i128::pow(BETA_BOUND,2)) {
        let scale_factor : f32 = ((i128::pow(BETA_BOUND,2)) / norm_sum) as f32;
